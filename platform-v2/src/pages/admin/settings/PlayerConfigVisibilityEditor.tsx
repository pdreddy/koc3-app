...
const PAGE_LABELS: Record<string, string> = {
  ...
  rules: 'Rules', announcements: 'Announcements', playerProfiles: 'Player Profiles',
  captainDashboard: 'Captain Dashboard', scoreEntry: 'Score Entry', adminDashboard: 'Admin Dashboard',
  playerImport: 'Player Import', tournamentSettings: 'Tournament Settings',
};
...
export function VisibilityEditor({ value, onChange }: { value: VisibilityMap; onChange: (next: VisibilityMap) => void }) {
  return (
    <Stack spacing={2}>
      ...
      <Grid container spacing={2}>
        {Object.entries(value).map(([pageId, level]) => (
          <Grid item xs={12} sm={6} key={pageId}>
            <Stack direction="row" spacing={2} alignItems="center">
              <Typography sx={{ flexGrow: 1 }}>{PAGE_LABELS[pageId] ?? pageId}</Typography>
              <Select size="small" value={level} onChange={(e) => onChange({ ...value, [pageId]: e.target.value as VisibilityLevel })} sx={{ minWidth: 180 }}>
